#!/usr/bin/env -S mise x node@22 -- node

// https://github.com/tmd-x/bitwarden-cli-bio/commit/71e600af3aaef45f022070f181fc2bc9ad7d57ab
// TODO: Remove when https://github.com/jeanregisser/bitwarden-cli-bio/pull/4 is merged
...
var package_default = {
  ...
  keywords: ["bitwarden", "cli", "biometrics", "touchid", "windows-hello"],
...
var IpcSocketService = class {
   ...
   */
  async connectToSocketPath(socketPath) {
  ...
  sendMessage(message) {
    ...
    logDebug(`Sending ${buffer.length} bytes (message: ${messageBytes.length} bytes)`);
...
// src/fingerprint.ts
function hkdfExpand(prk, info, outputLength) {
  const hashLen = 32;
  const n = Math.ceil(outputLength / hashLen);
  const okm = Buffer.alloc(n * hashLen);
  let prev = Buffer.alloc(0);
  for (let i = 1; i <= n; i++) {
    const hmac = crypto2.createHmac("sha256", prk);
    hmac.update(prev);
...
function getFingerprint(fingerprintMaterial, publicKey) {
  const keyFingerprint = crypto2.createHash("sha256").update(publicKey).digest();
...
var BiometricsStatus = /* @__PURE__ */ ((BiometricsStatus2) => {
  ...
  BiometricsStatus2[(BiometricsStatus2["NotEnabledInConnectedDesktopApp"] = 8)] =
    "NotEnabledInConnectedDesktopApp";
  BiometricsStatus2[(BiometricsStatus2["NativeMessagingPermissionMissing"] = 9)] =
...
var NativeMessagingClient = class {
  ...
  async encryptMessage(message) {
    ...
    const cipher = crypto3.createCipheriv("aes-256-cbc", sharedSecret.subarray(0, 32), iv);
  ...
  async handleSetupEncryption(message) {
    ...
    const decrypted = crypto3.privateDecrypt(
      {
  ...
  async handleEncryptedMessage(rawMessage) {
    ...
    if ("encryptionType" in rawMessage || "encryptedString" in rawMessage) {
      ...
      if (!crypto3.timingSafeEqual(mac, expectedMac)) {
        throw new Error("Message integrity check failed");
      }
      const decipher = crypto3.createDecipheriv("aes-256-cbc", encKey, iv);
...
function generateSessionKey() {
  const keyBytes = crypto4.randomBytes(64);
...
function encryptWithSessionKey(data, sessionKey) {
  ...
  const cipher = crypto4.createCipheriv("aes-256-cbc", encKey, iv);
  const encrypted = Buffer.concat([cipher.update(data), cipher.final()]);
  const hmac = crypto4.createHmac("sha256", macKey);
  hmac.update(iv);
  hmac.update(encrypted);
  const mac = hmac.digest();
...
function writeCliData(data) {
  ...
  fs.writeFileSync(dataPath, JSON.stringify(data, null, 2), { mode: 384 });
}
...
function generateAppId() {
  return `bwbio-${crypto5.randomUUID()}`;
}
async function attemptBiometricUnlock(options = {}) {
  const userId = options.userId || getActiveUserId();
  if (!userId) {
    logVerbose("Biometric unlock unavailable: No user ID available - please log in first");
  ...
  try {
    ...
    log(`Authenticate with ${getBiometricMethodName()} on Desktop app to continue...`);
  ...
  return new Promise((resolve2) => {
    ...
    const child =
      process.platform === "win32"
        ? spawn([getBwPath(), ...args2].join(" "), {
            stdio: "inherit",
            env,
            shell: true,
          })
        : spawn(getBwPath(), args2, { stdio: "inherit", env });
    child.on("error", (err) => {
...
`);
    writeLn("You can also pass the session key to any command with the `--session` option. ex:");
    writeLn(`$ bw list items --session ${sessionKey}`);
  }
  return 0;
}
async function main(args2) {
  if (args2.includes("--bwbio-version")) {
    writeLn(package_default.version);
    return 0;
  }
  ...
  const isUnlockCommand = args2[0] === "unlock";
  if (
    (process.env.BW_SESSION && !isUnlockCommand) ||
    process.env.BW_NOINTERACTION === "true" ||
    isPassthroughCommand(args2)
  ) {
